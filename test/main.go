...
func main() {
	...
	// 遍历文件夹
	targetType := []string{".go", ".py", ".xml", ".json", ".txt"}
	ignoreFile := []string{}
	ignorePath := []string{}
	ignoreType := []string{}
	files := []string{}
	err := GetAllFile("../gocode", &files, &targetType, &ignoreFile, &ignorePath, &ignoreType)
	if err != nil {
		log.Fatal(err)
	}
	// 批量修正文件换行符
	for _, v := range files {
		fmt.Println("path:", v)
		command := fmt.Sprintf("dos2unix %s", v)
